...
sed -i "/uci commit network/i\uci set network.lan.ipaddr='192.168.8.1'" $ZZZ              # IPv4 地址(openwrt后台地址)
...
sed -i 's/luci-theme-bootstrap/luci-theme-bootstrap/g' feeds/luci/collections/luci/Makefile   # 强制选择argon为默认主题选项
sed -i "/uci commit luci/i\uci set luci.main.mediaurlbase=/luci-static/bootstrap" $ZZZ        # 选择argon为默认主题

sed -i "/uci commit system/i\uci set system.@system[0].hostname='PingTouGe'" $ZZZ       # 修改主机名称为OpenWrt-123
